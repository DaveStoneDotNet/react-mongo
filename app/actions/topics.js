...
// This action creator returns a function, which will get executed by Redux-Thunk middleware.
// This function does not need to be pure, and thus allowed to have side effects, including executing asynchronous API calls.
export function createTopic(text) {
  ...
  return (dispatch, getState) => {
    ...
    // Conditional dispatch - If the topic already exists, make sure we emit a dispatch event.
    if (topic.topics.filter(topicItem => topicItem.id === id).length > 0) {

      // Currently there is no reducer that changes state for this. For production you would ideally have a message reducer that notifies the user of a duplicate topic.
    ...
    return voteService().createTopic({ id, data })
      .then((res) => {
        if (res.status === 200) {
          // We can actually dispatch a CREATE_TOPIC_SUCCESS on success, but I've opted to leave that out since we already did an optimistic update. We could return res.json();
